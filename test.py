...
def count_occurrences_in_text(word, text):
    ...
    while right <= len(text):
        ...
        if text[left:right].lower() == word.lower():
            # First let's check if the word is at the beginning of the text or
            if left == 0:
                # if the word is at the beginning of the text, we need to check if the next character is a space or a valid special characters
                if right == len(text) or text[right].isalpha() or text[right].isdigit():
                    count += 1
            if right == len(text):
                # if the word is at the end of the text, we need to check if the previous character is a space or a valid special characters
                if left == 0 or text[left - 1].isalpha() or text[left - 1].isdigit():
                    count += 1
            
            # if the word is in the middle of the text, we need to check if the previous and next characters are spaces or valid special characters
            if left != 0 and right != len(text):
                if (text[left - 1].isalpha() or text[left - 1].isdigit()) or (text[right].isalpha() or text[right].isdigit()):
                    count += 1
        
        # move the window by 1 step each time
        left += 1
        right += 1
    return count
